...
/*"review": {
    "is_approver": true,
    "task_db_id": "5f16ec25ea9741614e766c43",
    "status": "approved",
    "approved_by": "Sukant Priyadarshi",
    "approved_on": "21/07/2020 06:58:34 PM",
    "by": "Sukant Priyadarshi",
    "at": 1595338114000
}*/
...
class ViewController: UIViewController {
  ...
  @objc private func reload(_ sender: Any?) {
    ...
    let drawingData = jsonParser("webdrawing")!
    //let shapeModels: DrawsanaShapesModel = try! JSONDecoder().decode(DrawsanaShapesModel.self, from: drawingData)
    //print(shapeModels.shapes.map{ $0.canvasFormat })
    
    /*let annos: Annotations = try! JSONDecoder().decode(Annotations.self, from: drawingData)
    
    let anns = annos.annotations.map{ $0.shapeFormat }
    let dic = ["shapes": anns,
               "size" : [
                 375,
                 281.33333333333331
    ...
    if let drawingData = try? JSONSerialization.data(withJSONObject: dic, options: .prettyPrinted) {
  ...
  private func jsonParser(_ filename: String) -> Data! {
