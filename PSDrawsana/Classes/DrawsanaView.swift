...
public protocol DrawsanaViewDelegate: AnyObject {
  ...
  func drawsanaView(_ drawsanaView: DrawsanaView, onDoubleTap loc: CGPoint)
}

public extension DrawsanaViewDelegate {
  func drawsanaView(_ drawsanaView: DrawsanaView, onDoubleTap loc: CGPoint) {}
}
/**
...
public class DrawsanaView: UIView {
  ...
  private func commonInit() {
    ...
    panGR.onDoubleTap = { [weak self] loc in
      guard let ss = self else { return }
      ss.delegate?.drawsanaView(ss, onDoubleTap: loc)
    }
  ...
  public override func layoutSubviews() {
    ...
    let oldSize = drawing.size
    
    drawing.size = frame.size
    
    let scaleWidth = newSize.width / oldSize.width
    let scaleHeight = newSize.height / oldSize.height
    
    for shape in drawing.shapes {
      if let pen = shape as? PenShape {
        pen.segments = pen.segments.map{ PenLineSegment(a: CGPoint(x: $0.a.x * scaleWidth, y: $0.a.y * scaleHeight),
                                         b: CGPoint(x: $0.b.x * scaleWidth, y: $0.b.y * scaleHeight),
                                         width: $0.width * scaleWidth) }
      }
      
      if let rect = shape as? RectShape {
        rect.a = CGPoint(x: rect.a.x * scaleWidth, y: rect.a.y * scaleHeight)
        rect.b = CGPoint(x: rect.b.x * scaleWidth, y: rect.b.y * scaleHeight)
      }
      
      if let ellipse = shape as? EllipseShape {
        ellipse.a = CGPoint(x: ellipse.a.x * scaleWidth, y: ellipse.a.y * scaleHeight)
        ellipse.b = CGPoint(x: ellipse.b.x * scaleWidth, y: ellipse.b.y * scaleHeight)
      }
      
      if let line = shape as? LineShape {
        line.a = CGPoint(x: line.a.x * scaleWidth, y: line.a.y * scaleHeight)
        line.b = CGPoint(x: line.b.x * scaleWidth, y: line.b.y * scaleHeight)
      }
